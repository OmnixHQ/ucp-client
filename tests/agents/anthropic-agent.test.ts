/**
 * Anthropic adapter — full agent shopping flow.
 *
 * Simulates what an LLM agent does: calls tools in sequence through the
 * Anthropic adapter and verifies the results at each step.
 *
 * Run:
 *   npm run mock-server          # terminal 1
 *   INTEGRATION=true GATEWAY_URL=http://localhost:3002 \
 *     npx vitest run tests/agents/anthropic-agent.test.ts
 */
...
describe.skipIf(process.env['INTEGRATION'] !== 'true')(
  'Anthropic adapter — agent shopping flow',
  () => {
    ...
    it('tool definitions have correct Anthropic shape', () => {
      const tools = toAnthropicTools(agentTools, { catchErrors: true });
    ...
    it('completes full shopping flow: search → checkout → update → cancel', async () => {
      ...
      const products = (await executeAnthropicToolCall(agentTools, 'search_products', { query: 'roses' }, {
        catchErrors: true,
      })) as Array<{ id: string }>;
    ...
    it('throws when catchErrors is false', async () => {
      await expect(
        executeAnthropicToolCall([...agentTools, errorTool], 'failing_tool', {}),
